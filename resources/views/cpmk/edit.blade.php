        ...
        <div class="p-4 shadow-sm rounded bg-white">
            ...
            <form method="POST" action="{{ route('cpmk.update', $cpmk->id) }}">
                    ...
                    <select class="form-select @error('kode_cpl') is-invalid @enderror" id="kode_cpl" name="kode_cpl"
                        ...
                        @foreach ($cpls as $cpl)
                            <option value="{{ $cpl->kode_cpl }}"
                                {{ old('kode_cpl', $cpmk->kode_cpl) == $cpl->kode_cpl ? 'selected' : '' }}>
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control" id="kode_cpmk" name="kode_cpmk"
                        value="{{ old('kode_cpmk', $cpmk->kode_cpmk) }}" readonly>
                    ...
                    <select class="form-select @error('mata_kuliah') is-invalid @enderror" id="mata_kuliah"
                        ...
                        @foreach ($matakuliahs as $matakuliah)
                            <option value="{{ $matakuliah->kode_mk }}"
                                {{ old('mata_kuliah', $cpmk->mata_kuliah) == $matakuliah->kode_mk ? 'selected' : '' }}>
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control @error('deskripsi') is-invalid @enderror" id="deskripsi" name="deskripsi" rows="5"
                        placeholder="Masukkan deskripsi CPMK" required>{{ old('deskripsi', $cpmk->deskripsi) }}</textarea>
                    @error('deskripsi')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-3">
                    ...
                    <input type="number" step="0.01" class="form-control @error('bobot') is-invalid @enderror"
                        id="bobot" name="bobot" value="{{ old('bobot', $cpmk->bobot) }}" required>
                    @error('bobot')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const picSearchInput = document.getElementById("pic-search-input"),
                picSearchResultsTbody = document.getElementById("pic-search-results-tbody"),
                selectedPicTbody = document.getElementById("selected-pic-tbody"),
                picHiddenInputsContainer = document.getElementById("pic-hidden-inputs");
            ...
            function addPicHiddenInput(e) {
                picHiddenInputsContainer.querySelector(`input[name="pic_ids[]"][value="${e}"]`) || (e => {
                    const t = document.createElement("input");
                    t.type = "hidden", t.name = "pic_ids[]", t.value = e, picHiddenInputsContainer
                        .appendChild(t)
                })(e)
            }

            function removePicHiddenInput(e) {
                (e => {
                    const t = picHiddenInputsContainer.querySelector(`input[name="pic_ids[]"][value="${e}"]`);
                    t && t.remove()
                })(e)
            }

            function updatePicSearchResultsTable() {
                picSearchResultsTbody.querySelectorAll(".add-pic-btn").forEach(e => {
                    e.disabled = selectedPicIdSet.has(e.getAttribute("data-id"))
                })
            }

            function addPicToSelectedTable(e) {
                const t = String(e.id);
                if (selectedPicIdSet.has(t)) return;
                selectedPicIdSet.add(t);
                addPicHiddenInput(e.id);
                const n = document.createElement("tr");
                n.setAttribute("data-id", e.id), n.innerHTML =
                    `\n            <td>${e.nama}</td>\n            <td>${e.nidn}</td>\n            <td><button type="button" class="btn btn-danger btn-sm remove-selected-pic-btn" data-id="${e.id}">Hapus</button></td>\n        `,
                    selectedPicTbody.appendChild(n);
                updatePicSearchResultsTable()
            }

            function removePicFromSelectedTable(e) {
                const t = String(e);
                selectedPicIdSet.delete(t), removePicHiddenInput(e);
                const n = selectedPicTbody.querySelector(`tr[data-id="${e}"]`);
                n && n.remove(), updatePicSearchResultsTable()
            }

            function renderPicSearchResults(e) {
                picSearchResultsTbody.innerHTML = "", 0 === e.length ? picSearchResultsTbody.innerHTML =
                    '<tr><td colspan="3" class="text-center text-muted">Tidak ada dosen ditemukan.</td></tr>' : e
                    .forEach(e => {
                        const t = selectedPicIdSet.has(String(e.id)),
                        ...
                        n.innerHTML =
                            ...
                            picSearchResultsTbody.appendChild(n)
                    })
            }
            async function fetchDosens(e) {
                if ("" === e.trim()) {
                ...
                try {
                    const t = await fetch(`{{ route('dosen.search.json') }}?q=${encodeURIComponent(e)}`);
                    if (!t.ok) throw new Error(`Gagal fetch: ${t.status}`);
                    const n = await t.json();
                    renderPicSearchResults(n)
                } catch (e) {
                    console.error("Fetch error:", e), picSearchResultsTbody.innerHTML =
            ...
            picSearchInput.addEventListener("input", debounce(e => fetchDosens(e.target.value), 350)),
                picSearchResultsTbody.addEventListener("click", e => {
                    e.target.classList.contains("add-pic-btn") && (addPicToSelectedTable({
                        id: e.target.dataset.id,
                        nama: e.target.dataset.nama,
                        nidn: e.target.dataset.nidn
                    }), e.target.disabled = !0)
                }), selectedPicTbody.addEventListener("click", e => {
                    e.target.classList.contains("remove-selected-pic-btn") && removePicFromSelectedTable(e
                        .target.dataset.id)
                });
