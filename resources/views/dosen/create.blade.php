...
@section('content')
    <div class="container-fluid">
        ...
        {{-- Notifikasi Sukses --}}
        @if (session('success_dosen_ajax'))
            {{-- Menggunakan key session yang berbeda untuk AJAX --}}
                ...
                <form method="POST" action="{{ route('dosen.store') }}" id="addDosenForm">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" id="kontak" name="kontak"
                            value="{{ old('kontak') }}"> {{-- Mengubah type menjadi text untuk nomor telepon yang mungkin ada + atau spasi --}}
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" id="kompetensi" name="kompetensi"
                            value="{{ old('kompetensi') }}" required>
                        <div class="invalid-feedback" data-field="kompetensi"></div>
                    </div>
                    <div class="mb-3">
                        <label for="prodi" class="form-label">Prodi</label> {{-- Mengubah nama field agar konsisten jika ada relasi --}}
                        <select class="form-select" id="prodi" name="prodi" required> {{-- Mengubah class dan nama --}}
                        ...
                        <div class="invalid-feedback" data-field="prodi"></div>
                    </div>
                    <div class="mt-4">
                            ...
                            if (error.data.errors) { // Jika ada error validasi
                                ...
                                Object.keys(error.data.errors).forEach(key => {
                                    ...
                                    if (field) {
                                        ...
                                        const feedbackElement = field.parentElement
                                            .querySelector(
                                                `.invalid-feedback[data-field="${key}"]`);
                                        if (feedbackElement) {
                                            feedbackElement.textContent = error.data.errors[key]
                                                .join(', ');
                                        }
                                    }
                                    errorMessage +=
                                        `<li>${error.data.errors[key].join(', ')}</li>`;
                                });
                                errorMessage += '</ul>';
                            }
                        }
...
@section('styles')
    <style>
        ...
        .invalid-feedback {
            display: none;
            /* Sembunyikan secara default, tampilkan saat ada error */
            width: 100%;
            margin-top: 0.25rem;
            font-size: .875em;
            color: #dc3545;
        ...
        .is-invalid~.invalid-feedback {
            display: block;
            /* Tampilkan jika field terkait is-invalid */
        }
