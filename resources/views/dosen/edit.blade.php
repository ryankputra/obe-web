            ...
            <form method="POST" action="{{ route('dosen.update', $dosen->id) }}">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control @error('gelar') is-invalid @enderror" id="gelar" name="gelar" 
                           value="{{ old('gelar', $dosen->gelar) }}" required>
                    @error('gelar')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control @error('kompetensi') is-invalid @enderror" id="kompetensi" name="kompetensi" 
                           value="{{ old('kompetensi', $dosen->kompetensi) }}" required>
                    @error('kompetensi')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select @error('prodi_id') is-invalid @enderror" id="prodi_id" name="prodi_id" required>
                        ...
                        {{-- @php $prodis = \App\Models\Prodi::all(); // Contoh pengambilan data prodi --}}
                        {{-- @foreach($prodis as $item) --}}
                        {{-- <option value="{{ $item->id }}" {{ old('prodi_id', $dosen->prodi_id ?? $dosen->prodi) == $item->id ? 'selected' : '' }}>{{ $item->nama_prodi }}</option> --}}
                        {{-- @endforeach --}}
                        {{-- Untuk sementara, menggunakan opsi statis dan menyesuaikan dengan field $dosen->prodi jika prodi_id tidak ada --}}
                        <option value="Informatika" {{ old('prodi_id', $dosen->prodi_id ?? $dosen->prodi) == 'Informatika' ? 'selected' : '' }}>Informatika</option>
                        <option value="Sistem Informasi" {{ old('prodi_id', $dosen->prodi_id ?? $dosen->prodi) == 'Sistem Informasi' ? 'selected' : '' }}>Sistem Informasi</option>
                        <option value="Rekayasa Perangkat Lunak" {{ old('prodi_id', $dosen->prodi_id ?? $dosen->prodi) == 'Rekayasa Perangkat Lunak' ? 'selected' : '' }}>Rekayasa Perangkat Lunak</option>
                    </select>
                    @error('prodi_id')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
